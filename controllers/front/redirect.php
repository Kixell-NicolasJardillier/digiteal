...
class DigitealRedirectModuleFrontController extends ModuleFrontController
{
    ...
    public function postProcess()
    {
        ...
        $endpoint = DigitealRest::getEndPoint().'/api/v1/payment-request/pay-button/execute';
        $orderTotal = $cart->getOrderTotal(true, Cart::BOTH);
        $digitealAmountInCents = (float) $orderTotal * 100;

        $cancelUrl = Context::getContext()->link->getPageLink($order_page);
        $confirmationURL = Context::getContext()->link->getModuleLink('digiteal', 'confirmation', ['cart_id' => $cart->id], true);
     ...
     * @return string
     */
    private function digitealPageLink($controller, $query = [])
    {
